...
## 1. Цель
Каждый завершённый тест автоматически уезжает на узел `https://storage.lan` без действий оператора; при отсутствии сети — копится и доезжает позже. Пациента заводит оператор вручную (ФИО/код на планшете); сшивку с человеком делает identity-метод узла — справочник пациентов на планшет не выгружается (см. §3, пересмотрено 2026-07-10).
...
## 3. Пациент (ПЕРЕСМОТРЕНО 2026-07-10 — справочник с узла убран)
Изначально §3 требовала тянуть `GET /api/persons` в кэш `node_persons` и выбирать пациента из
списка узла. От этого отказались по итогам согласования (см. PROGRESS «Вопросы к контрактам» N19–N20):
- `GET /api/persons` на узле закрыт сессией оператора (`require_role("researcher")`), ключом планшета
  недоступен — даже с верным ключом там `401`. Отдавать базу пациентов на планшет решили не отдавать.
- **Штатный и единственный путь**: оператор вводит ФИО/код на планшете вручную; запись помечается
  `patient_ref: {local_name}`. Сшивку с человеком делает identity-метод узла
  (`storage-core/ingester/adapters/planshet.py`, ветка `local_name`) — не планшет.
- `patient_ref: {node_person_id, display_code}` и store `node_persons` в приложении не используются.
- К записи теста добавляются поля: `timepoint` (1|2|null — селектор «Точка», запоминается до смены
  пациента) и `condition` (null|'pre_load'|'post_load' — селектор «Нагрузка», по умолчанию null).
